...
```{r}
If you create a custom stopword list, this changes the perfomance of the model in several ways. The first is that it can improve performance by filtering out extra words and reducing noise in data. The second is that it transforms text data into numerical format which is effective in creating datasets with higher dimensions. Therefore, removing stopwords can improve efficiency in the model. If text data contains domain specific words, a stopword list can be customized so that it keeps the words that are necessary. 
```
...
data <- data.frame(mb.metadata = documents, year = year)
data_1920s <- subset(data, year >= 1920 & year < 1930)
corpus <- Corpus(VectorSource(data_1920s$documents))
...
It changes the model for the topics generated which are based on text data from the 1920s documents. This will also lead to topics which are most asssociated with the 1920s, but the change depends on the text data in those documents. 
...
data_bb.metadata <- data.frame(documents = documents, year = year)
bb.metadata <- subset(data, year >= 1880 & year < 1900)
corpus <- Corpus(VectorSource(bb.metadata$documents))
corpus <- tm_map(corpus, content_transformer(tolower))
corpus <- tm_map(corpus, removePunctuation)
corpus <- tm_map(corpus, removeNumbers)
corpus <- tm_map(corpus, removeWords, stopwords("Crime"))
corpus <- tm_map(corpus, stripWhitespace)
...
library(tm)
library(topicmodels)
dtm <- DocumentTermMatrix(bb.metadata$text)
lda <- LDA(dtm, k = 5)
topics <- tidy(lda)
ggplot(topics, aes(timestamp, topics)) + geom_line() + facet_wrap(~data)
...
lda <- LDA(dtm, k = 5)
topics <- tidy(lda)
ggplot(topics, aes(timestamp, topics)) + geom_line() + facet_wrap(~promotion)
...
lda <- LDA(dtm, k = 5)
topics <- tidy(lda)
ggplot(topics, aes(timestamp, topics)) + geom_line() + facet_wrap(~reception)


We learn by doing this that R provides many benefits for linear and nonlinear modeling. This allows for improved statistical tests like time analysis, classification, and clustering. Furthermore, it can help organize data that is unstructured and visualize and reproduce that data. 
...
extraneous_data <- bb.metadata %>% filter(category == "extraneous articles")
tokenize(bb.metadata)
dtm <- DocumentTermMatrix(extraneous_data$text)
lda <- LDA(dtm, k = 5)
topics <- tidy(lda)
...
sentiment_data <- bb.metadata %>% unnest_tokens(word, text) %>%
...
sentiment_ts <- ts(sentiment_data$sentiment)
cpt <- cpt.mean(sentiment_ts)
...
I focused on the research question of whether there is a change over time in promotion language. After looking at the bb.metadata and attempting to create a topic model, I can see that there is a way to detect changes over time. This can be done by cleaning the text data and converting it to a format which will allow analysis through tokenization, using tidytext to perform sentiment analysis to understand the text data, applying topic modeling like LDA to identify themes in how promotion language changed over time, using changepoint to detect changes, and text mining. 
