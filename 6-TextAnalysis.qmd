...
>
```{r}
Step 1. meta <- as.data.frame(sotu_meta) converts sotu_meta into a dataframe. This dataframe has metadata related to SOTU or the State of the Union speeches.

Step 2. file_paths <- sotu_dir(dir = "sotu_files") retrieves directory paths for the SOTU files.

Step 3. sotu_texts <- readtext(file_paths) reads text from SOTU files into an object in R.

Step 4. sotu_whole <- sotu_meta %>% arrange(president) %>% bind_cols(sotu_texts) %>% as_tibble() organizes and combines metadata with texts from the SOTU speeches. The metadata is arranged by whoever the president is, combines metadata with texts of speeches from that president, and converts the result into a tibble.

Step 5. tidy_sotu <- sotu_whole %>% unnest_tokens(word, text) %>% anti_join(stop_words) tokenizes the text into words, removes stop words like "the" "and" and others from the text, and results in a clean data frame. Once this is done, it is ready for topic modeling to begin. 
...
> More modern presidents' state of the union addresses are shorter because 

(@) Filter the dataset to address this discrepancy and the recreate these statistics: 
```{r}
filtered_sotu <- tidy_sotu %>% filter(n >= 100)
speech_stats <- filtered_sotu %>%
  group_by(president) %>% 
  summarize(
    avg_words = mean(),
    max_words = max(),
...
longest_speech <- speech_stats %>% arrange(desc(max_words)) %>% topenv(1)
shortest_speech <- speech_stats %>% arrange(desc(min_words)) %>% topenv(1)
```
